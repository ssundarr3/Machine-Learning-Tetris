...
function dropPiece(board, col, pieceID, pieceName) {
    // function prototypes

    var gameNotOver = dropPieceAtCol(board, col, pieceID, pieceName);

    if(gameNotOver){
        return removeClears(board);
    }
    else return -1;
}
...
function dropPieceAtCol(board, col, pieceID, pieceName){
    // function prototypes
    var piece = PIECES[pieceID];


    for(var i = 0; i <= BOARD_HEIGHT - piece.length; ++i){
        if(!canDropPieceAt(board, i, col, pieceID, pieceName)){
            if(i == 0) return false;
    ...
    if(canDropPieceAt(board, BOARD_HEIGHT - piece.length, col, pieceID, pieceName)){
...
function canDropPieceAt(board, row, col, pieceID, pieceName){
    var piece = PIECES[pieceID];
    ...
    if (secondLevel) {
        ...
        for (var i = 0; i < pieceIDs.length; i++) {
            for (var j = 0; j <= BOARD_WIDTH - (PIECES[pieceIDs[i]])[0].length; j++) {
                ...
                numCleared = dropPiece(board4, j, pieceIDs[i], 3);
                if ( numCleared == -1 ) {
                    // continue;
                    score = -999999;
                }
                    ...
                    //console.log("CalculateFitness: " + calculateFitness(board4, numCleared));
...
function findBest(board, piece) {
    ...
    for (var i = 0; i < pieceIDs.length; i++) {
        for (var j = 0; j <= BOARD_WIDTH - PIECES[pieceIDs[i]][0].length; j++) {
            ...
            var numCleared = dropPiece(board2, j, pieceIDs[i], 3);
            if ( numCleared == -1 ) {
                continue;
            }
